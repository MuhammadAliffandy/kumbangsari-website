...
const SideBar = (props) => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            setCurrentPath(window.location.pathname.split('/dashboard/')[1]);
        }
    }, []);
    ...
    return(
        <>    
            ...
            <li className={`${currentPath == 'gallery' ? 'text-PRIMARY-500' : "text-TEXT-1" } hover:text-PRIMARY-500`}><a href="/dashboard/gallery">{ 'Galeri'}</a></li>
            <li className={`${currentPath == 'news' ? 'text-PRIMARY-500' : "text-TEXT-1" } hover:text-PRIMARY-500`}><a href="/dashboard/news">{ 'Berita'}</a></li>
            <li className={`${currentPath == 'shop' ? 'text-PRIMARY-500' : "text-TEXT-1" } hover:text-PRIMARY-500`}><a href="/dashboard/shop" > { "Belanja"}</a></li>
            </ul>
            ...
            <AppAnimationButton className='w-auto'>
                    <AppButton
                        className={`${props.isDrawer ? 'w-[100%]' : ''} px-[20px] py-[6px]  text-[12px] bg-SECONDARY-100 text-STATE-RED-DARKEN font-poppins rounded-[6px]`}
                        text={'Logout'} 
                        type = {'button'}
                        onClick={()=>{
                        push('/auth/signin')
